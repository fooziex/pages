...
# get login
preg_match('/^(.+)_\d+_\w{2}$/',$response['trans']['session_id'],$login);
$login=$login[1];
...
$payment_query = mysql_query("REPLACE INTO payu VALUES('"
	       ...
	       . mysql_real_escape_string($response['trans']['pos_id'])."','"
	       ...
	       . mysql_real_escape_string($response['trans']['desc2'])."','"
...
if($response['trans']['status'] == 99) {
	$user_query = mysql_query("DELETE FROM user WHERE login='"
	            . mysql_real_escape_string($login)."'"
	);
